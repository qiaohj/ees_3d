...
void Scenario::initSimulations(sqlite3 *conf_db, sqlite3 *env_db, int p_id, string p_target, bool p_overwrite,
    ...
    if (p_id == -1) {
        ...
        sql = "SELECT * FROM simulations WHERE is_run=1 and evo_type=1 and (species_evo_type=2 or species_evo_type=1) and nb='NARROW' and species_evo_level=0 order by random()";
    } else {
