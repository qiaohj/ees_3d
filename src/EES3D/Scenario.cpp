...
void Scenario::initSimulations(sqlite3 *conf_db, sqlite3 *env_db, int p_id, string p_target, bool p_overwrite,
    ...
    if (p_id == -1) {
        ...
        sql = "SELECT * FROM simulations WHERE is_run=1 and evo_type=1 and nb='NARROW' and global_id in (27790,25837,24109,54786,19759,13651,17569,18517,4497,4847,9898,11847,40440,23724) order by random()";
    } else {
