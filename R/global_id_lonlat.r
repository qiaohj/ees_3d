...
mask_df<-inner_join(global_id_lonlat, min_temp, by=c("global_id"="ID"))
#ggplot(mask_df %>% filter(Y==0))+geom_point(aes(x=lon, y=lat, color=MIN_TEMP))
mask_df<-inner_join(mask_df, max_temp, by=c("global_id"="ID", "Y"="Y"))
mask_df<-inner_join(mask_df, max_prec, by=c("global_id"="ID", "Y"="Y"))
