...
for (i in c(1:length(items))){
  ...
  source_rda<-sprintf("%s/%s/center_density.rda", target_dir, item)
  if (!file.exists(source_rda)){
    print(paste("skip", source_rda))
    next()
  }
  target_rda<-sprintf("%s/%s/center_density_new_dist.rda", target_dir, item)
  ...
  print("Calculating the new distance ...")
  df<-df%>%dplyr::rowwise()%>%
    dplyr::mutate(
